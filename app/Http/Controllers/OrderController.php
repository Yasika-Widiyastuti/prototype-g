...
class OrderController extends Controller
{
    ...
    public function storeReview(Request $request, $orderId)
    {
        ...
        $validated = $request->validate([
            ...
            'reviews.*.comment.max' => 'Komentar maksimal 1000 karakter.',
        ]);
        ...
        if ($reviewedCount > 0) {
            $message = "Berhasil memberikan ulasan untuk {$reviewedCount} produk.";
            if ($skippedCount > 0) {
                $message .= " {$skippedCount} produk sudah pernah diulas.";
            }
            return redirect()->route('profile.orders.show', $order->id)
