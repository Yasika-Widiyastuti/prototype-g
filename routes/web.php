...
// ==========================
// PRODUCT DETAIL & CART (Auth Required)
// ==========================
...
// ==========================
// CHECKOUT (Auth + User Access ONLY - NO VerifiedUser Middleware!)
// ==========================
Route::middleware(['auth', 'user.access'])
    ->prefix('checkout')
    ->name('checkout.')
    ->group(function () {
        // Halaman checkout - bisa diakses semua user yang login
        // Verifikasi status akan dicek di controller/view, bukan di middleware
        Route::get('/', [CheckoutController::class, 'index'])->name('index');
        
        // AJAX operations - tetap bisa diakses untuk update cart
        ...
        // Payment routes - HANYA yang ini yang perlu verifikasi
        // Kita akan cek di controller menggunakan custom logic
...
Route::middleware(['auth', 'admin'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::post('/audit-logs/cleanup', [AdminAuditController::class, 'cleanup'])->name('audit.cleanup');
    ...
    Route::post('/orders/{order}/confirm-pickup', [AdminOrderController::class, 'confirmPickup'])->name('orders.confirm-pickup');
    Route::post('/orders/{order}/confirm-return', [AdminOrderController::class, 'confirmReturn'])->name('orders.confirm-return');
    ...
    // ✅ TAMBAHKAN ROUTE INI untuk viewing documents
    Route::get('users/{user}/document/{type}', [UserController::class, 'viewDocument'])
        ->name('documents.view')
        ->where('type', 'ktp|kk');

    // Di dalam middleware admin
    Route::get('/admin/documents/{user}/{type}', function($userId, $type) {
        $user = \App\Models\User::findOrFail($userId);
        
        if ($type === 'ktp') {
            $path = storage_path('app/' . $user->ktp_path);
        } elseif ($type === 'kk') {
            $path = storage_path('app/' . $user->kk_path);
        } else {
            abort(404);
        }
    ...
    })->middleware(['auth', 'admin'])->name('admin.documents.view');
