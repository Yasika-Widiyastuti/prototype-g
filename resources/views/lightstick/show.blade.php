...
@auth
    @if(auth()->user()->isVerified())
        <form method="POST" action="{{ route('cart.add.lightstick', $product->id) }}">
            @csrf
        ...
        <div class="bg-white p-8 rounded-xl shadow-lg mb-12">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    @auth
                        <form action="{{ route('cart.add.lightstick', $product['id']) }}" method="POST" id="addToCartForm">
                            @csrf
                            {{-- *** PERUBAHAN UTAMA DI SINI: type diubah dari "submit" menjadi "button" *** --}}
                            <button type="button" 
...
@push('scripts')
<script>
// *** PERUBAHAN UTAMA DI SINI: Mendengarkan event 'click' pada tombol, BUKAN 'submit' pada form. ***
document.getElementById('addToCartBtn').addEventListener('click', function(e) {
    ...
    // Submit form via fetch (Hanya akan terjadi sekali)
    fetch(form.action, {
        method: 'POST',
