        ...
        <div class="bg-white p-6 rounded-lg shadow">
            ...
            <form method="POST" action="{{ route('admin.payments.update-status', $payment) }}" class="space-y-4">
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                    <h5 class="font-semibold text-blue-900 mb-2">Order Terkait</h5>
                    <p class="text-blue-700">{{ $payment->order->order_number }} - Rp {{ number_format($payment->order->total_amount, 0, ',', '.') }}</p>
                    <a href="{{ route('admin.orders.show', $payment->order) }}" class="text-blue-600 hover:text-blue-900 text-sm">Lihat detail →</a>
                </div>
                @endif
